...
export const KNOWLEDGE_BASE: KbEntry[] = [
  ...
  },
  {
    ...
    content:
      "Our privacy policy details what we collect (account info, pledge data, shipping addresses, browser metadata), how we use it (account servicing, fulfillment, fraud prevention), what we share with processors (PayPal, Mentom Payments, Divinity Payments, Whop — each handles card data on their own PCI-compliant infrastructure; we never see card numbers), and your rights (access, deletion, opt-out of marketing). You can delete your own account at Dashboard → Settings → Privacy & Security → Delete my account. For backers, deletion is immediate and permanent — there is no grace window and it cannot be undone. Uncharged pledges are cancelled, rewards on completed pledges are forfeited (creators are released from fulfilling them), and your profile and shipping addresses are erased; pledge amounts and dates are retained for tax and audit purposes. Anyone who has ever taken a campaign live goes through admin review instead of instant deletion, and the request cannot be approved while any backer is still unfulfilled — every backer must be marked shipped or delivered first. Prelaunch pages do not count as launched. A deleted account's email address cannot be reused to sign up again.",
    link: "/privacy",
  },
  {
    id: "trust-safety",
