...
export function ProductionOrderView({ productionOrderStats, projectId }: ProductionOrderViewProps) {
  ...
  const handleExportCSV = () => {
    ...
    lines.push(`Production Progress,${productionOrderStats.fulfillmentPercentage}%`);
    lines.push("");
  ...
  return (
    <div className="space-y-6">
      {/* Production Order Progress Overview */}
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-1 bg-card/50 backdrop-blur border-border/50">
          ...
          <CardContent className="flex flex-col items-center">
            <CircularProgress
              value={productionOrderStats.fulfillmentPercentage}
              size={180}
              strokeWidth={12}
              color="green"
            />
            <div className="mt-4 text-center">
              <p className="text-lg font-semibold">
                {productionOrderStats.shippedBackers} of {productionOrderStats.totalBackers} backers
              </p>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center p-4 rounded-xl bg-muted/30 border border-border/50">
                ...
                <p className="text-2xl font-bold">{productionOrderStats.statusBreakdown.notStarted}</p>
                <p className="text-xs text-muted-foreground">Not Started</p>
              </div>
              <div className="text-center p-4 rounded-xl bg-blue-500/10 border border-blue-500/20">
                <div className="w-4 h-4 rounded-full bg-blue-500 mx-auto mb-2" />
                <p className="text-2xl font-bold text-blue-500">{productionOrderStats.statusBreakdown.inProgress}</p>
                <p className="text-xs text-muted-foreground">In Progress</p>
              </div>
              <div className="text-center p-4 rounded-xl bg-amber-500/10 border border-amber-500/20">
                <div className="w-4 h-4 rounded-full bg-amber-500 mx-auto mb-2" />
                <p className="text-2xl font-bold text-amber-500">{productionOrderStats.statusBreakdown.shipped}</p>
                <p className="text-xs text-muted-foreground">Shipped</p>
              </div>
              <div className="text-center p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                <div className="w-4 h-4 rounded-full bg-emerald-500 mx-auto mb-2" />
                <p className="text-2xl font-bold text-emerald-500">{productionOrderStats.statusBreakdown.delivered}</p>
              ...
              <div className="flex h-4 rounded-full overflow-hidden bg-muted/30">
                {productionOrderStats.statusBreakdown.delivered > 0 && (
                  <div
                    ...
                    style={{
                      width: `${(productionOrderStats.statusBreakdown.delivered / productionOrderStats.totalBackers) * 100}%`,
                    }}
                  />
                )}
                {productionOrderStats.statusBreakdown.shipped > 0 && (
                  <div
                    ...
                    style={{
                      width: `${(productionOrderStats.statusBreakdown.shipped / productionOrderStats.totalBackers) * 100}%`,
                    }}
                  />
                )}
                {productionOrderStats.statusBreakdown.inProgress > 0 && (
                  <div
                    ...
                    style={{
                      width: `${(productionOrderStats.statusBreakdown.inProgress / productionOrderStats.totalBackers) * 100}%`,
                    }}
                  />
                )}
              </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Complete breakdown of all items needed for your production order
              </p>
            </div>
            <Button
