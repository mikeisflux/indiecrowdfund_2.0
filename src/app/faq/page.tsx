...
const faqData: Record<string, { question: string; answer: string }[]> = {
  ...
  payments: [
    { question: "What are the platform fees?", answer: "We charge a 3% platform fee on all successfully funded projects, plus payment processing fees. With Stripe, total fees are ~6% (3% platform + 2.9% + $0.30 processing). With Chain2Pay, total fees are ~5.5% (3% platform + 2.5% processing). With DivinityCoin, total fees are ~9% (3% platform + 6% partner fee). Creators choose their payment processor when setting up a project." },
    { question: "Which payment methods are accepted?", answer: "We accept all major credit cards, debit cards, Apple Pay, and Google Pay via Stripe. Chain2Pay accepts credit and debit cards through their secure hosted checkout page. Backers can also use DivinityCoin credits, which they purchase in advance and can use across multiple creator platforms." },
    { question: "What is Chain2Pay?", answer: "Chain2Pay is a secure payment processor with our lowest total fees at ~5.5% (3% platform + 2.5% processing). It uses a redirect-based checkout where you enter your card details on Chain2Pay's secure hosted page — IndieCrowdfund never handles your card data (SAQ A PCI compliant). Behind the scenes, payments settle as USDC on Polygon, but backers simply pay with their regular credit or debit card in USD." },
    { question: "What is DivinityCoin?", answer: "DivinityCoin is a universal credit system that allows backers to purchase credits in advance, then use those credits to back campaigns on IndieCrowdfund and other partner platforms. 1 DivinityCoin = $1 USD. It offers backers flexibility while providing creators with secure, pre-funded payments. Not cryptocurrency — simply platform credit." },
    { question: "When is my card charged?", answer: "For Stripe payments, your card is only charged when a campaign successfully reaches its funding goal. For Chain2Pay, your card is charged at the time of pledge through Chain2Pay's secure checkout. For DivinityCoin, credits are purchased in advance and captured when the campaign funds successfully." },
    { question: "What if my payment fails?", answer: "If your Stripe payment fails when the campaign funds, we automatically retry up to 3 times over 9 days (once every 3 days) before marking the pledge as failed. Chain2Pay payments are processed immediately at checkout, so failures are handled in real-time on their payment page. DivinityCoin credits are pre-funded, so payment failures don't occur." },
    { question: "When do creators receive their funds?", answer: "For Stripe payments, we use Stripe Connect so payouts start as soon as the campaign is funded, though it can take up to 14 days depending on if you have a new account with Stripe. For Chain2Pay and DivinityCoin payments, a settlement will be applied to the creator's bank account within 14 business days after the campaign ends." },
  ...
  security: [
    { question: "How is my payment information protected?", answer: "All payments are processed through PCI-compliant processors. Stripe (Level 1 PCI Service Provider) handles card payments via tokenized forms. Chain2Pay uses a redirect-based hosted checkout (SAQ A PCI compliant) — your card data is entered on their secure page and never touches our servers. CSRF protection is enforced on all payment initiations, and all communication is HTTPS-only. We never store full credit card details." },
