...
export async function GET(req: NextRequest) {
  try {
    ...
    retailersOrdersLogger.error({ err: formatError(error) }, "Error fetching retailer orders:");
    return NextResponse.json(
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      summary: {
        totalQuantity,
        originalAmount: Number(totalOriginalAmount),
        wholesaleAmount: Number(totalWholesaleAmount),
        savings: Number(totalOriginalAmount) - Number(totalWholesaleAmount),
        shippingCost: Number(shippingCost),
        grandTotal: Number(totalWholesaleAmount) + Number(shippingCost),
      },
    });
  } catch (error) {
    retailersOrdersLogger.error({ err: formatError(error) }, "Error creating retailer order:");
    return NextResponse.json(
