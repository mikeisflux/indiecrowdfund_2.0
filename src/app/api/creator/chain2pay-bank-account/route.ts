...
// GET - Check if user has a Chain2Pay bank account configured
export async function GET() {
  try {
    ...
    const bankAccount = await db.chain2PayBankAccount.findUnique({
    ...
    console.error("Error fetching Chain2Pay bank account:", error);
    return NextResponse.json(
...
// POST - Create or update Chain2Pay bank account
export async function POST(req: NextRequest) {
  try {
    ...
    // If projectId is provided, verify the user is the project owner
    if (projectId) {
    ...
    // Upsert bank account
    const bankAccount = await db.chain2PayBankAccount.upsert({
      where: { userId: session.user.id },
      update: {
    ...
    console.error("Error saving Chain2Pay bank account:", error);
...
// DELETE - Remove Chain2Pay bank account
export async function DELETE() {
  try {
    ...
    await db.chain2PayBankAccount.delete({
      where: { userId: session.user.id },
    });

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("Error deleting Chain2Pay bank account:", error);
    return NextResponse.json(
