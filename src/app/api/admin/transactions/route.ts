...
async function querySettlements(
  ...
  return settlements.map((s: typeof settlements[number]) => ({
    ...
    userId: s.bankAccount?.user?.id ?? "unknown",
    userName: s.bankAccount?.user?.name ?? null,
    userEmail: s.bankAccount?.user?.email ?? null,
    userImage: s.bankAccount?.user?.image ?? null,
