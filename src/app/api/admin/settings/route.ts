...
export async function GET() {
  try {
    ...
    try {
      settings = await db.platformSettings.findUnique({
        ...
        select: {
          ...
          maverickEnabled: true, maverickApiToken: true, maverickWebhookSecret: true,
          maverickDbaId: true, maverickTerminalId: true, maverickEnvironment: true,
          autoPayouts: true,
    ...
    if (!settings) {
      try {
        // Create default settings via upsert so two concurrent admin
        // dashboard loads don't race on `id: "default"` and one
        // P2002s → 500.
        settings = await db.platformSettings.upsert({
          where: { id: "default" },
          create: { id: "default" },
          update: {},
          select: {
    ...
    const maskedSettings: Record<string, unknown> = {
      ...
      maverickApiToken: settings.maverickApiToken ? "••••••••" : null,
      maverickWebhookSecret: settings.maverickWebhookSecret ? "••••••••" : null,
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const secretFields = [
      ...
      'maverickApiToken', 'maverickWebhookSecret',
    ...
    const allowedFields: Record<string, string[]> = {
      ...
      payments: [
        ...
        "maverickEnabled", "maverickApiToken", "maverickWebhookSecret",
        "maverickDbaId", "maverickTerminalId", "maverickEnvironment",
        "autoPayouts",
