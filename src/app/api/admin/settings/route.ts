...
export async function GET() {
  try {
    ...
    try {
      settings = await db.platformSettings.findUnique({
        ...
        select: {
          ...
          printingComicsApiKey: true, printingComicsWebhookSecret: true, printingComicsEnvironment: true,
          autoPayouts: true,
    ...
    const maskedSettings: Record<string, unknown> = {
      ...
      nmiWebhookSecret: settings.nmiWebhookSecret ? "••••••••" : null,
      printingComicsApiKey: settings.printingComicsApiKey ? "••••••••" : null,
      printingComicsWebhookSecret: settings.printingComicsWebhookSecret ? "••••••••" : null,
  ...
  try {
    ...
    const allowedFields: Record<string, string[]> = {
      ...
      payments: [
        ...
        "printingComicsApiKey", "printingComicsWebhookSecret", "printingComicsEnvironment",
        "autoPayouts",
