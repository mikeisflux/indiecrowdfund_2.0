...
const tabContent: Record<string, { title: string; description: string; fields: FieldInfo[] }> = {
  ...
  'payment': {
    ...
    fields: [
      ...
      { name: 'Payment Processor Selection', required: true, description: 'Choose your payment processor during project setup. You can switch processors even on live campaigns (NSFW projects cannot switch to Stripe).', tips: 'Compare fees: Stripe ~6%, DivinityCoin ~9%. Choose based on your needs and content type.' },
      { name: 'Settlement Account', required: true, description: 'For Stripe, connect via Stripe Connect. For DivinityCoin, enter your US bank account details (bank name, account number, routing number). Details are AES-256 encrypted.', tips: 'Double-check all bank details. Settlement accounts can be configured in your IndieKit dashboard under Settings > Payments.' },
    ]
  },
  'promotion': {
