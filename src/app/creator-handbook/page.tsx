...
const tabContent: Record<string, { title: string; description: string; fields: FieldInfo[] }> = {
  'verification': {
    ...
    fields: [
      ...
      { name: 'Payment Account Verification', required: true, description: 'Add your PayPal payout email (for standard campaigns) or complete DivinityCoin bank account setup (for NSFW/adult content campaigns). PayPal automatically verifies your account when processing your first payout.', tips: 'For PayPal: go to Dashboard > Settings > Payments and add your PayPal email. For DivinityCoin: have your US bank routing and account number ready.' },
  ...
  'payment': {
    ...
    fields: [
      ...
      { name: 'PayPal (Primary Processor)', required: true, description: 'The default payment processor for standard (non-NSFW) campaigns. Backers pay via PayPal Advanced Checkout — credit/debit cards, PayPal wallet, and Venmo. Full payments land in our platform PayPal account, and we pay out your net earnings automatically.', tips: 'Platform fee 3% + PayPal fees (3.49% + $0.49/txn) ≈ 6.5% total. To receive payouts, add your PayPal email in Dashboard > Settings > Payments. Not available for NSFW/adult content.' },
      ...
      { name: 'Payment Processor Selection', required: true, description: 'Choose your payment processor during project setup. You can switch processors even on live campaigns (NSFW projects must use DivinityCoin).', tips: 'Compare fees: PayPal ~6.5%, DivinityCoin ~9%. Choose based on your needs and content type.' },
      { name: 'Payout Account', required: true, description: 'For PayPal campaigns, add your PayPal payout email in Dashboard > Settings > Payments. For DivinityCoin, enter your US bank account details (bank name, account number, routing number). Details are AES-256 encrypted.', tips: 'Double-check your PayPal email address. PayPal payouts are sent automatically after your campaign funds. DivinityCoin bank details can be configured in your IndieKit dashboard under Settings > Payments.' },
    ]
  },
  'promotion': {
  ...
  'marketplace': {
    ...
    fields: [
      ...
      { name: 'Platform Fee', required: false, description: 'Marketplace uses PayPal: 3% platform fee + PayPal fees (3.49% + $0.49/txn). NSFW content uses DivinityCoin instead.', tips: 'Example: $10 sale → you receive ~$9.30. Add your PayPal payout email in Dashboard > Settings > Payments to receive automatic payouts.' },
