...
const tabContent: Record<string, { title: string; description: string; fields: FieldInfo[] }> = {
  ...
  'payment': {
    ...
    fields: [
      ...
      { name: 'DivinityCoin (Primary Processor for New Campaigns)', required: false, description: 'DivinityCoin is the active processor for new campaigns. Backers fund a DivinityCoin wallet and pay from it, or charge a card directly at checkout. Supports all content types including adult/NSFW. Both All-or-Nothing and Keep-It-All flows.', tips: 'Fees: roughly 3% + $0.30/txn (DivinityCoin) + 3% (platform) ≈ 6% total. Settlements within 14 business days.' },
      { name: 'PayPal Processor', required: false, description: 'Use PayPal Advanced Checkout instead of DivinityCoin. Backers can pay with PayPal wallet OR card-only (no PayPal account needed). Best for general-audience campaigns where backer trust in PayPal\'s name matters more than processor flexibility.', tips: 'Fees: roughly 3.49% + $0.49/txn (PayPal) + 3% (platform). PayPal does NOT support adult/NSFW content — DivinityCoin is required for those.' },
      ...
      { name: '180-Day Rolling Reserve', required: false, description: 'For certain DivinityCoin campaigns: 10% of your gross campaign revenue may be held in escrow for 180 days as protection against chargebacks. Calculated BEFORE fees are deducted. Applied on a case-by-case basis for new/high-risk creators during review. Applies to crowdfunding pledges and IndieKit after-sales; does NOT apply to digital marketplace sales.', tips: 'Plan your fulfillment cash flow around an effective payout of ~67.5% of gross while the reserve is held. Reserve releases automatically after 180 days assuming no outstanding chargebacks.' },
