...
const tabContent: Record<string, { title: string; description: string; fields: FieldInfo[] }> = {
  ...
  'payment': {
    ...
    fields: [
      ...
      { name: 'PaymentCloud', required: false, description: 'High-risk friendly merchant account on the NMI gateway. Backers enter their card directly on your campaign page — the card is securely tokenized in the browser via Collect.js (PAN never touches our servers) and only charged when the campaign hits its goal. Supports both All or Nothing and Keep It All. 100% guaranteed NSFW friendly.', tips: '4% + $0.25/txn PaymentCloud + 3% platform fee ≈ 7.5% total. 100% NSFW friendly — supports all content types. Funds settle to your bank account on the merchant account schedule.' },
      { name: 'PayPal (Primary Processor)', required: true, description: 'The default payment processor for standard (non-NSFW) campaigns. Backers pay via PayPal Advanced Checkout — credit/debit cards, PayPal wallet, and Venmo. All pledges are collected into IndieCrowdfund\'s PayPal account, and your net earnings are deposited directly to your bank account at settlement — no PayPal account required.', tips: 'Platform fee 3% + PayPal fees (3.49% + $0.49/txn) ≈ 6.5% total. Add your bank account in Dashboard > Settings > Payments to receive payouts. Not available for NSFW/adult content.' },
      { name: 'DivinityCoin', required: false, description: 'Alternative payment sub-processor where backers enter their credit or debit card at checkout. DivinityCoin handles payment processing seamlessly — backers see a secure card payment form with Visa, Mastercard, Amex, and Discover logos.', tips: '3% partner fee + $0.30/txn + 3% platform fee ≈ 6.5% total. Supports all content types including NSFW/adult. Settlement to your bank account within 14 business days.' },
      { name: 'Whop', required: false, description: 'Alternative embedded checkout processor supporting all content types including NSFW/adult. Backers pay via Whop\'s embedded form. Whop is a Keep It All only processor — payments are collected immediately.', tips: '3% platform fee + ~3% Whop fee ≈ 6% total. Supports NSFW/adult content. Settlement to your payout account. Only available with Keep It All campaign style.' },
      { name: 'Campaign Funding Model', required: true, description: 'All or Nothing: Your campaign only receives funds if it reaches its goal — otherwise all authorizations are released and backers pay nothing. Keep It All: You keep all pledges regardless of whether you hit your goal; backers are charged immediately.', tips: 'NSFW/adult projects must use Keep It All (alternative processors do not support payment holds). All or Nothing is recommended for most campaigns as it builds backer trust.' },
      { name: 'Payment Processor Selection', required: true, description: 'Choose your payment processor during project setup. First select your campaign funding model (All or Nothing vs Keep It All), then pick your processor. You can switch processors even on live campaigns.', tips: 'Compare fees: PayPal ~6.5%, DivinityCoin ~6.5%, Whop ~6%, PaymentCloud ~7.5%. All or Nothing: PayPal, DivinityCoin, or PaymentCloud. Keep It All: any processor. NSFW projects can use PaymentCloud, DivinityCoin, or Whop.' },
      { name: 'Payout Account', required: true, description: 'For PayPal campaigns, add your bank account details in Dashboard > Settings > Payments. For DivinityCoin, enter your US bank account details (bank name, account number, routing number). All payout account details are AES-256 encrypted.', tips: 'PayPal payouts go directly to your bank account — no PayPal account needed. DivinityCoin bank details can also be configured in your IndieKit dashboard under Settings > Payments.' },
    ]
  },
  'promotion': {
