...
const tabContent: Record<string, { title: string; description: string; fields: FieldInfo[] }> = {
  ...
  'payment': {
    ...
    fields: [
      ...
      { name: 'PaymentCloud (Primary Processor)', required: true, description: 'High-risk friendly merchant account on the NMI gateway. Backers enter their card directly on your campaign page — the card is securely tokenized in the browser via Collect.js (PAN never touches our servers) and only charged when the campaign hits its goal. Supports both All or Nothing and Keep It All. 100% guaranteed NSFW friendly — all content types accepted.', tips: '4% + $0.38/txn PaymentCloud + 3% platform fee ≈ 7.5% total. Funds settle to your bank account on PaymentCloud\'s standard merchant schedule.' },
      { name: '180-Day Rolling Reserve', required: false, description: 'PaymentCloud holds 10% of your gross campaign revenue in escrow for 180 days as protection against chargebacks. The reserve is calculated BEFORE processing and platform fees are deducted. Auto-applied to any campaign that raises over $2,500. Also applied to new or high-risk creators on a case-by-case basis decided during project review. Applies to crowdfunding pledges and IndieKit after-sales (post-campaign add-on charges); does NOT apply to digital marketplace sales.', tips: 'The reserve releases automatically to your payout balance after the 180-day window if no outstanding chargebacks remain. We absorb what we can on smaller campaigns — the rest is held by the processor and released back to you. Plan your fulfillment cash flow around an effective payout of ~67.5% of gross until the reserve releases.' },
