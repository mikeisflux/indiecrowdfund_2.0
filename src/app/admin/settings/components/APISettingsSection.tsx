...
interface APISettingsSectionProps {
  apiKeys: APIKey[];
  onCreateKey?: (name: string, environment: string) => void;
...
export function APISettingsSection({
  ...
  const handleCreateKey = () => {
    if (onCreateKey && newKeyName) {
      onCreateKey(newKeyName, newKeyEnvironment);
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    variant="outline"
                    size="icon"
                    onClick={() => copyToClipboard(`${window.location.origin}/api/verify-id/callback`)}
                  >
                    <Copy className="h-4 w-4" />
                  </Button>
                </div>
